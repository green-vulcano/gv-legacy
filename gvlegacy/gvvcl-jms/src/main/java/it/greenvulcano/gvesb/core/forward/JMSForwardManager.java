...
public class JMSForwardManager implements ConfigurationListener, ShutdownEventListener
{
    ...
    *
    */
    public static synchronized JMSForwardManager instance() throws Exception
    {
        if (instance == null) {
    ...
    */
    ...
    public void configurationChanged(ConfigurationEvent evt)
    {
        ...
        if ((evt.getCode() == ConfigurationEvent.EVT_FILE_LOADED) && evt.getFile().equals(JMS_FORWARD_FILE_NAME)) {
        	try {
                init();
            }
            catch (Exception exc) {
