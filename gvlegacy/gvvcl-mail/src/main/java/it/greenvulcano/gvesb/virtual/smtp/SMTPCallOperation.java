...
public class SMTPCallOperation implements CallOperation
{
    ...
    private static final Logger logger                     = LoggerFactory.getLogger(SMTPCallOperation.class);
        ...
        try {        	
            ...
            if ((nodeList != null) && (nodeList.getLength() > 0)) {
                ...
                for (int i = 0; i < nodeList.getLength(); i++) {
                    ...
                    if (name.contains(".host")) {
                        logger.debug("INIT -- Logging-in to host: " + value);
                        serverHost = value;
                    }
                    else if (name.contains(".user")) {
                        logger.debug("INIT -- Logging-in as user: " + value);
                        loginUser = value;
                    }
            ...
            if ((protocolHost != null) || (protocolUser != null)) {
                ...
                if (loginPassword != null) {
                    serverProps.setProperty("mail.protocol.password", loginPassword);
                }
            }
    ...
    private void sendMail(GVBuffer gvBuffer) throws Exception
    {
        try {
            ...
            try {
                ...
                logger.debug("Generated mail subject: " + subjectText);
