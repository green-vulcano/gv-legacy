...
/**
 * Class defining a call to retweetStatus (Retweet) on Twitter.
 * 
 * @version 4.0.0 - Feb 2017
 * @author GreenVulcano Developer Team
 */
public class TwitterRetweetStatusCallOperation extends TwitterSocialCallOperation {
    private static Logger logger = LoggerFactory.getLogger(TwitterRetweetStatusCallOperation.class);
	...
	public void init(Node node) throws InitializationException {
		try{
			super.init(node);
			statusId = XMLConfig.get(node, "@statusId");
		}
        catch (Exception exc) {
            logger.error("ERROR TwitterRetweetStatusCallOperation[" + getName() + "] initialization", exc);
	...
	public GVBuffer perform(GVBuffer gvBuffer) throws ConnectionException,
		...
		try {
        	...
        	logger.error("ERROR TwitterRetweetStatusCallOperation[" + getName() + "] execution", exc);
